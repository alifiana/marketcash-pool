...
var cryptoNightMarketCash = multiHashing['cryptonight-marketcash'];
...
if (shareBuffer[0] >= 5) {
     // log('info',logSystem,'Block version 5 seen');
            hash = cryptoNightMarketCash(convertedBlob);
        }
        else {
...
//        log('info',logSystem,'%d  submit-block-string:%s',[job.height, shareBuffer.toString('hex')]);
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
	    ...
	    if (!noncePattern.test(params.nonce)) {
			...
			if (!shareAccepted){
                sendReply('will take a Low difficulty share');
