...
```bash
git clone https://github.com/MarketCashME/marketcash-pool.git pool
cd pool
npm update
```
...
"poolServer": {
    ...
    /* Set difficulty on miner client side by passing <address> param with .<difficulty> postfix
       minerd -u MrWXK2c7MygdWeyYz2NRix9sjTsAY8AXhNakJpNDftGSAijJNrL9gmxMh9UyA32a7kgVVNxYzeGMmeLbqCAE2hr9E15APaA.5000 */
    "fixedDiff": {
        "enabled": true,
...
"blockUnlocker": {
    ...
    "Faucet": 0.5, //0.1% Official donation for MarketCash faucet. http://faucet.marketcash.io
...
var cryptonatorWidget = ["MKT-BTC", "MKT-USD", "MKT-EUR"];
...
* You now need to run two instance of the daemon and connect them to each other (without a connection to another instance the daemon will not accept RPC requests)
  * Run first instance with `./marketcashd --p2p-bind-port 28080 --allow-local-ip`
  * Run second instance with `./marketcashd --p2p-bind-port 5011 --rpc-bind-port 5010 --add-peer 0.0.0.0:28080 --allow-local-ip`
...
```bash
curl 127.0.0.1:32267/json_rpc -d '{"method":"getblockheaderbyheight","params":{"height":100}}'
```
...
* Run the daemon using the following code:

```./marketcashd --rpc-bind-ip 0.0.0.0 --enable-cors "*" --enable_blockexplorer
```

* Change the following line in the pool's frontend config.json:

```
var api_blockexplorer = "http://daemonhost.com:32267";
```
